...
(function IiId() {
    for (let i = 0; i < bottomLength; i++) {
        ...
        document.getElementById(`${i}`).onclick = () => {//为每个li绑定点击事件
            img[i].style.zIndex = 1;
            //存储每次点击的i
            if(imgArrLength >1){
                imgArrLength = 0;
            }
            imgArr[imgArrLength] = i;
            imgArrLength++;
            // console.log(`imgArr[${i}] = ${imgArr[imgArr.length]}`)
            //使上一个图片的z-index为0
        }
    }
}())
...
// const bottomLiClosure = (perviousValue) => {
//     let i = perviousValue;
//     return () => {
...
// const perviousValue = bottomLiClosure();
